...
function music:new(jsonPath)
  ...
  return setmetatable(
    {
      defaultControlls = t.controlls,
      events = t.events,
      releaseTime = t.release_time or 0.1,
      isLoop = isLoop,
      dontChangeControlls = t.dont_change_controlls or false,
      -- now
      controlls = table2.copy(t.controlls),
      noteIds = {},
      picthes = {},
      velocities = {},
      tempTime = 0,
...
function music:readEvents()
  ...
  for id, event in ipairs(self.events) do
    ...
    elseif event.type == "control" then
      if self.tempTime <= event.time and event.time < self.time then
        self:controlChange(event.name, event.value)
        table.insert(events, {"controlChange", event.name, event.value})
        self.controlls[event.name] = event.value

        if event.once then
          table2.removeItem(self.events, event)
        end
      end
    end
  end
...
function music:reset()
  ...
  self.controlls = table2.copy(self.defaultControlls)
...
function music:nowControlls()
  if self.dontChangeControlls then
    return {}
  end
